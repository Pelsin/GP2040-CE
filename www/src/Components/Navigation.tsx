...
const Navigation = () => {
	...
	const [isRebooting, setIsRebooting] = useState<number | null>(null); // null because we want the button to assume untouched state
	...
	const handleReboot = async (bootMode: number) => {
		setIsRebooting(bootMode);
		await WebApi.reboot(bootMode);
		setIsRebooting(-1);
	};

	const updateButtonLabels = (e: React.ChangeEvent<HTMLSelectElement>) => {
	...
	return (
		<Navbar collapseOnSelect bg="dark" variant="dark" expand="md" fixed="top">
			...
			<Navbar.Collapse id="basic-navbar-nav">
				<Nav className="me-auto">
					...
					<NavDropdown title="Links">
						<NavDropdown.Item
							as={NavLink}
							to={{ pathname: "https://gp2040-ce.info/" }}
							target="_blank"
						>
							Documentation
						</NavDropdown.Item>
						<NavDropdown.Item
							as={NavLink}
							to={{
								pathname: "https://github.com/OpenStickCommunity/GP2040-CE",
							}}
							target="_blank"
						>
							Github
					...
					<NavDropdown title="DANGER ZONE" className="btn-danger danger-zone">
						<NavDropdown.Item as={NavLink} to="/reset-settings">
							Reset Settings
						</NavDropdown.Item>
					</NavDropdown>
				</Nav>
			...
			<Modal show={show} onHide={handleClose}>
				...
				<Modal.Body>
					{isRebooting === -1
						? "Done rebooting, this browser tab can now be closed."
						: "Select a mode to reboot to"}
				</Modal.Body>
				<Modal.Footer>
					<Button
						variant="secondary"
