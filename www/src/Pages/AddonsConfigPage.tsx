...
const ANALOG_PINS = [-1, 26, 27, 28, 29];
	...
	async function checkRead() {
		if (count < 3) {
			...
			try {
				...
				document.getElementById("ps4alert").textContent =
					"ERROR: Could not verify required files";
			}
		}
	}
	setTimeout(checkRead, 1000);
...
const schema = yup.object().shape({
	...
	analogAdcPinX: yup
		.number()
		.label("Analog Stick Pin X")
		.validatePinWhenValue("AnalogInputEnabled"),
	analogAdcPinY: yup
		.number()
		.label("Analog Stick Pin Y")
		.validatePinWhenValue("AnalogInputEnabled"),
			...
			{({ handleSubmit, handleChange, values, errors }) => (
				<Form noValidate onSubmit={handleSubmit}>
					...
					<Section title="BOOTSEL Button Configuration">
						...
						>
							<p>
								Note: OLED might become unresponsive if button is set, unset to
								restore.
							</p>
							<FormSelect
					...
					<Section title="Analog">
						<div id="AnalogInputOptions" hidden={!values.AnalogInputEnabled}>
							<p>
								Available pins: {ANALOG_PINS.filter((p) => p !== -1).join(", ")}
							</p>
							<Row className="mb-3">
								<FormSelect
					...
					<Section title="Player Number (X-INPUT ONLY)">
						...
						>
							<p>
								<strong>
									WARNING: ONLY ENABLE THIS OPTION IF YOU ARE CONNECTING
									MULTIPLE GP2040-CE DEVICES WITH PLAYER NUMBER ENABLED
								</strong>
							</p>
							<Row className="mb-3">
								<FormControl
					...
					<Section title="SOCD Cleaning Mode Selection Slider">
						...
						>
							<Row className="mb-3">
								<p>
									Note: PS4, PS3 and Nintendo Switch modes do not support
									setting SOCD Cleaning to Off and will default to Neutral SOCD
									Cleaning mode.
								</p>
								<FormSelect
									label="SOCD Slider Mode Default"
					...
					<Section title="PS4 Mode">
						<div id="PS4ModeOptions" hidden={!values.PS4ModeAddonEnabled}>
							<Row>
								...
								<p>
									Please upload the 3 required files and click the "Verify &
									Save" button to use PS4 Mode.
								</p>
							</Row>
							<Row className="mb-3">
					...
					<Section title="Wii Extension Configuration">
						...
						>
							<Row>
								...
								<p>
									Classic/Classic Pro: Both Analogs and D-Pad Supported. B = B1,
									A = B2, Y = B3, X = B4, L = L1, ZL = L2, R = R1, ZR = R2,
									Minus = S1, Plus = S2, Home = A1
								</p>
								<p>Nunchuck: Analog Stick Supported. C = B1, Z = B2</p>
								<p>
									Guitar Hero Guitar: Analog Stick Supported. Green = B1, Red =
									B2, Blue = B3, Yellow = B4, Orange = L1, Strum Up = Up, Strum
									Down = Down, Minus = S1, Plus = S2
								</p>
							</Row>
							<Row className="mb-3">
								<FormControl
