	...
	hotkeyF1: yup.array().of(
		yup.object({
			...
			mask: yup
				.number()
				.required()
				.oneOf(HOTKEY_MASKS.map((o) => o.value))
				.label("Hotkey action"),
		})
	),
	hotkeyF2: yup.array().of(
		yup.object({
			action: yup
				.number()
				.required()
...
export default function SettingsPage() {
	...
	const buttonLabelS1 =
		BUTTONS[buttonLabelType][swapTpShareLabels ? "A2" : "S1"];
							...
							<Section title="Hotkey Settings">
								...
								{HOTKEY_MASKS.map((o, i) => (
									<Form.Group key={`hotkey-${i}`} className="row mb-3">
										...
										<div className="col-sm-2">
											<Form.Select
												name={`hotkeyF1[${i}].action`}
												className="form-select-sm"
												value={values?.hotkeyF1 && values?.hotkeyF1[i]?.action}
												onChange={handleChange}
												isInvalid={
													errors?.hotkeyF1 && errors?.hotkeyF1[i]?.action
												}
											>
												{HOTKEY_ACTIONS.map((o, i) => (
													<option key={`f1-option-${i}`} value={o.value}>
														{o.label}
													</option>
												))}
											</Form.Select>
								...
								{HOTKEY_MASKS.map((o, i) => (
									<Form.Group key={`hotkey-${i}`} className="row mb-3">
										...
										<div className="col-sm-2">
											<Form.Select
												name={`hotkeyF2[${i}].action`}
												className="form-select-sm"
												value={values?.hotkeyF2 && values?.hotkeyF2[i]?.action}
												onChange={handleChange}
												isInvalid={
													errors?.hotkeyF2 && errors?.hotkeyF2[i]?.action
												}
											>
												{HOTKEY_ACTIONS.map((o, i) => (
													<option key={`f2-option-${i}`} value={o.value}>
														{o.label}
													</option>
												))}
											</Form.Select>
