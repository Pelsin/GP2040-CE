	...
	const renderError = (button) => {
		if (buttonMappings[button].error === errorType.required) {
			return (
				...
				>{`${BUTTONS[buttonLabelType][button]} is required`}</span>
			);
			...
			return (
				<span key="conflict" className="error-message">{`Pin ${
					buttonMappings[button].pin
				} is already assigned to ${conflictedMappings.join(", ")}`}</span>
			);
			...
			return (
				<span
					key="invalid"
					className="error-message"
				>{`Pin ${buttonMappings[button].pin} is invalid for this board`}</span>
			);
			...
			return (
				<span
					key="used"
					className="error-message"
				>{`Pin ${buttonMappings[button].pin} is already assigned to another feature`}</span>
			);
		}
